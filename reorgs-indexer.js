...
module.exports = class ERC20Indexer {
  ...
  async reduceEvent(event) {
    ...
    if (from !== ZERO_ADDRESS) {
      ...
      if (this.balances[from] == 0) {
        delete this.balances[from];
      }
    }
  ...
  async saveEvent(event) {
    if (!_.last(this.eventsBlocks) || _.last(this.eventsBlocks).hash !== event.blockHash) {
    ...
    _.last(this.eventsBlocks).transfers.push(event.returnValues);
  }
  ...
  async undoBlocks() {
    while (this.eventsBlocks.length > 0) {
      const lastEventsBlock = _.last(this.eventsBlocks);
